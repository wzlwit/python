# File: astrocrash09.py
# Version 1.1.0:
#   + Add debris object to game | Author: Jim-Kristian Malmhaug
# About: Space shooter game
...
class Debris(Wrapper):
    """ An debris which floats across the screen. """
    image = games.load_image("debris.bmp")
    ...
    def die(self):
        ...
        # if all debris and asteroids are gone, advance to next level
        if Debris.total <= 0 and Asteroid.total <= 0:
            self.game.advance()
        if self.debris_hit <= 0:
            Debris.total -= 1
            super(Debris, self).die()
        else:
            self.debris_hit -= 1  # Reduce necessary hit points to kill debris
...
class Asteroid(Wrapper):
    ...
    def die(self):
        ...
        # if all debris and asteroids are gone, advance to next level
        if Asteroid.total <= 0 and Debris.total <= 0:
            self.game.advance()
...
class Game(object):
    ...
    def advance(self):
        ...
        # create new asteroids and debris
