...
class Application(Frame):
    ...
    def create_widgets(self):
        ...
        # create description label, not going to need to access this label, so no need to assign the Label object to a variable
        Label(self, text="Choose your favorite movie types").grid(
            row=0, column=0, sticky=W)

        # create instruction label
        Label(self, text="Select all that apply: ").grid(
            row=1, column=0, sticky=W)

        # create Comedy check button, and add a var to this control (associate the check button’s status (selected or unchecked) with the likes_comedy attribute)
